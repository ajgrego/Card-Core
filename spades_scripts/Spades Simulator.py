...
class Card:
    suit: int   # 0..3
    rank: int   # 2..14
    def str(self): return f"{SUITS[self.suit]}{self.rank}"

def deck():
    return [Card(s, r) for s in range(4) for r in RANKS]
...
class GameLogger:
    ...
    def log_deal(self, hands, scores, seats=("N","E","S","W")):
        ...
        payload = {
            ...
            "hands": [[c.str() for c in h] for h in hands]
        }
        self._write(payload)
    ...
    def log_play(self, trick_id, seat, legal_moves, chosen, lead_suit, spades_broken, belief=None):
        self._write({
            ...
            "legal_moves":[c.str() for c in legal_moves], "chosen":chosen.str(),
            "belief":belief or {}
        })
    ...
    def log_trick_end(self, trick_id, winner_seat, cards_played):
        self._write({
            ...
            "cards":[(s, c.str()) for s,c in cards_played]
        })
...
def legal_moves(hand: List[Card], lead_suit: int, spades_broken: bool):
    if lead_suit is None:
        ...
        non_spades = [c for c in hand if c.suit != 3]
        if non_spades:
            return non_spades
        return hand
...
def greedy_bot_choose(hand, lead_suit, spades_broken):
    ...
    following = [c for c in moves if lead_suit is not None and c.suit == lead_suit]
    if following: return min(following, key=lambda c:c.rank)
    non_spades = [c for c in moves if c.suit != 3]
    if non_spades: return max(non_spades, key=lambda c:c.rank)
    return min(moves, key=lambda c:c.rank)

# --- Example self-play loop stub (play phase only) ---
def play_one_hand(logger: GameLogger, rng: random.Random):
    d = deck(); rng.shuffle(d)
    hands = [sorted(d[i*13:(i+1)*13], key=lambda c:(c.suit,c.rank)) for i in range(4)]
    ...
    for seat in range(4):
        hand_feats = {
            "spade_len": sum(1 for c in hands[seat] if c.suit==3),
            "aces": sum(1 for c in hands[seat] if c.rank==14),
        }
    ...
    for trick_id in range(13):
        ...
        for i in range(4):
            ...
            if lead_suit is None: lead_suit = move.suit
            if move.suit == 3: spades_broken = True
...
LOG_PATH = os.path.join(PROJECT_ROOT, "spades_data", "spades_logs.jsonl")
