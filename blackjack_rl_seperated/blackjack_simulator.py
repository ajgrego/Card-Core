'''
Credit for this simulator goes to dennis-ho's project at https://github.com/dennis-ho/blackjack-simulator


BUG Found:
Anthony Grego: I had to fix a bug in the project where setting a parameter for the number of decks would result
               in an error because it wsa originally received as a string rather than an integer. 
               
               I edited lines: 322 and 323
               
               fix:
                - Before:
                    parser.add_argument('--hands', type=float, help='Number of hands to play', default=100)
                    parser.add_argument('--decks', type=float, help='Number of decks to use in a shoe', default=8)
                - Afer: 
                    parser.add_argument('--hands', type=int, help='Number of hands to play', default=100)
                    parser.add_argument('--decks', type=int, help='Number of decks to use in a shoe', default=8)
                reason:
                    Python does not allow for a list to be multiplied by a float so i changed the type to integer to 
                    resolve conflicts when running the program with parameters.
'''
...
class Table:
    ...
    def clone(self):
        "Deep copy of the table of independent hands"
        return copy.deepcopy(self)
    
    def is_terminal(self):
        "Check if the current state is terminal"
        return self.curr_idx is None
...
def simulate_episode_rollout(state, policy):
  
    if state.is_terminal():
        return state.results()['win']
...
def mc_value(table_state, N=200, policy=basic_strategy):

    total = 0.0
    for _ in range(N):
        sim_table = table_state.clone()
        total += simulate_episode_rollout(sim_table, policy)
    return total / float(N)


def mc_best_action(table_state, N=150, policy=basic_strategy):
    ...
    evs = {}
    for a in actions:
        sim = table_state.clone()
        sim.do_action(a)

        if sim.is_terminal():
            evs[a] = sim.results()['win']
            continue
    ...
    return best_action, evs[best_action], evs
...
def main():
    ...
    parser.add_argument('--hands', type=int, help='Number of hands to play', default=100)
    parser.add_argument('--decks', type=int, help='Number of decks to use in a shoe', default=8)
    parser.add_argument('--pen', type=float, help='Deck penetration (number of decks played before shuffling)', default=6.5)
    ...
    while hands_played < int(args.hands):
        ...
        while not table.shuffle_pending:
            ...
            ev_estimates = mc_value(table, N=200)
            print(f"MC EV from initial state (hand {hands_played+1}): {ev_estimates:.3f}")
            logging.debug('MC EV from initial state: %.3f', ev_estimates)
            ...
            while table.curr_idx is not None:
                ...
                if len(actions) > 0:
                    action, best_ev, action_evs = mc_best_action(table, N=100)
                    print("MC EV Per Action;", action_evs)
                    print("MC Best Action:", action, "with EV:", round(best_ev,3))
                else:
                    action = None
